...
# A writer killed mid-sift must leave a heap the next locker repairs.  gdb
# single-steps one add through this module's code, copying the backing file at
# every state change; each copy is then opened and read (which recovers the dead
# writer's lock), and its min-heap checked: a permutation of the used slots,
# every heap_pos pointing back at its heap entry, and every parent's count at
# most its children's.
...
Data::TopK::Shared->new($path, $cap, $ks)->add($key);
EOF
close $v;
...
# Warm-up insert: a count-1 key rises from the last leaf to the root.
seed('insert', $CAP - 1, 3);
check('insert', [snapshots('insert', 'fresh')]);

# Bump: the root's count passes its equal-count children and sinks to a leaf.
seed('bump', $CAP, 3);
my ($used, undef, $heap) = heap_state(raw("$dir/bump.tk"));
my $root = $heap->[0];
my $key = do {
    my $buf = raw("$dir/bump.tk");
    my ($slots_off) = unpack 'x40 Q<', $buf;
    my $at = $slots_off + $root * (32 + (($KS + 7) & ~7));
    my $kl = unpack "x@{[$at + 24]} V", $buf;
    substr $buf, $at + 32, $kl;
};
like $key, qr{\Ak\d+\z}, "bump targets the root key ($key)";
check('bump', [snapshots('bump', $key)]);
is(Data::TopK::Shared->new("$dir/bump.tk", $CAP, $KS)->estimate($key), 4, "bump: the add bumped the root key to 4");

# Eviction: a new key replaces the root and sinks it.
seed('evict', $CAP, 3);
check('evict', [snapshots('evict', 'fresh')]);
